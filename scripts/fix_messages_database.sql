-- Script de correction pour la table messages et ses dépendances
-- Créé le 01/06/2025
...
CREATE POLICY IF NOT EXISTS "Les utilisateurs peuvent voir leurs messages envoyés ou reçus"
  ON public.messages
  FOR SELECT
...
CREATE POLICY IF NOT EXISTS "Les utilisateurs peuvent mettre à jour les messages qu'ils ont reçus"
  ON public.messages
  FOR UPDATE
...
-- 5. Vérification de la structure de la table conversations
-- Assurez-vous que la table conversations a les bonnes colonnes

-- 5.1 Vérifier si la colonne provider_id existe et la créer si nécessaire
DO $$ 
DECLARE
...
BEGIN
  ...
  -- Vérifier si participant1_id existe
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'conversations' 
  ...
  -- Si provider_id n'existe pas mais participant1_id existe, renommer
  IF NOT column_exists AND participant1_exists THEN
    EXECUTE 'ALTER TABLE public.conversations RENAME COLUMN participant1_id TO provider_id';
    RAISE NOTICE 'Colonne participant1_id renommée en provider_id';
  END IF;
END $$;

-- 5.2 Vérifier si la colonne client_id existe et la créer si nécessaire
DO $$ 
DECLARE
  column_exists BOOLEAN;
...
BEGIN
  ...
  -- Vérifier si participant2_id existe
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'conversations' 
  ...
  -- Si client_id n'existe pas mais participant2_id existe, renommer
  IF NOT column_exists AND participant2_exists THEN
    EXECUTE 'ALTER TABLE public.conversations RENAME COLUMN participant2_id TO client_id';
    RAISE NOTICE 'Colonne participant2_id renommée en client_id';
  END IF;
END $$;

-- 6. Mise à jour des politiques RLS pour la table conversations

-- 6.1 Activer RLS sur la table conversations si nécessaire
DO $$ 
DECLARE
  rls_enabled BOOLEAN;
BEGIN
  -- Vérifier si RLS est activé sur la table conversations
  SELECT EXISTS (
    SELECT 1
    FROM pg_class c
  ...
  IF NOT rls_enabled THEN
    ...
    RAISE NOTICE 'Row Level Security activé sur la table conversations';
  END IF;
END $$;

-- 6.2 Supprimer les anciennes politiques si elles existent
...
-- 6.3 Vérifier si les colonnes provider_id et client_id existent avant de créer les politiques
DO $$ 
DECLARE
...
BEGIN
  ...
  -- Créer les politiques seulement si les deux colonnes existent
  IF provider_id_exists AND client_id_exists THEN
    -- Politique SELECT
    EXECUTE 'CREATE POLICY "Les utilisateurs peuvent voir leurs conversations" 
             ON public.conversations 
             FOR SELECT 
             USING (auth.uid() = provider_id OR auth.uid() = client_id)';
    
    -- Politique INSERT
    EXECUTE 'CREATE POLICY "Les utilisateurs peuvent créer des conversations" 
             ON public.conversations 
             FOR INSERT 
             WITH CHECK (auth.uid() = provider_id OR auth.uid() = client_id)';
    
    -- Politique UPDATE
    EXECUTE 'CREATE POLICY "Les utilisateurs peuvent mettre à jour leurs conversations" 
             ON public.conversations 
             FOR UPDATE 
             USING (auth.uid() = provider_id OR auth.uid() = client_id)';
    
    RAISE NOTICE 'Politiques RLS créées pour la table conversations';
  ELSE
    RAISE NOTICE 'Impossible de créer les politiques RLS: colonnes provider_id ou client_id manquantes';
  END IF;
END $$;

-- 7. Création d'un déclencheur pour mettre à jour la colonne read_at

-- 7.1 Créer la fonction de déclencheur avec chemin de recherche fixe
...
-- 7.2 Vérifier si le déclencheur existe déjà et le créer si nécessaire
DO $$ 
DECLARE
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'set_read_at'
  ) INTO trigger_exists;
  
  -- Créer le déclencheur s'il n'existe pas
  IF NOT trigger_exists THEN
    EXECUTE 'CREATE TRIGGER set_read_at
             BEFORE UPDATE ON public.messages
             FOR EACH ROW
             WHEN (NEW.read = TRUE AND OLD.read = FALSE)
             EXECUTE FUNCTION public.update_read_at()';
    RAISE NOTICE 'Déclencheur set_read_at créé avec succès';
  ELSE
    RAISE NOTICE 'Le déclencheur set_read_at existe déjà';
  END IF;
END $$;

-- 8. Ajout d'un index pour améliorer les performances des requêtes fréquentes
