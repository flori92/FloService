 ...
 * Ce script crée les tables manquantes et applique les politiques RLS adaptées
 */
...
async function fixSecurityIssues() {
  ...
  try {
    ...
    if (providerProfilesResult.rows.length === 0) {
      ...
      `);
      console.log('Table provider_profiles créée avec succès');
    } else {
      console.log('La table provider_profiles existe déjà');
    }
    ...
    // Vérifier si la table provider_profiles existe maintenant
    const providerProfilesResult2 = await client.query(`
      SELECT 1 FROM pg_tables 
      WHERE schemaname = 'public' AND tablename = 'provider_profiles'
    `);
    
    if (providerProfilesResult2.rows.length > 0) {
      console.log('Application des politiques RLS pour provider_profiles...');
      
      // Supprimer les politiques existantes
      await client.query(`DROP POLICY IF EXISTS "Les utilisateurs peuvent modifier leur propre profil" ON public.provider_profiles`);
      ...
      console.log('Politiques RLS pour provider_profiles appliquées avec succès');
    }
