...
export const getProfileWithProviderData = async (userId: string): Promise<{
  ...
  try {
    ...
    if (userId.startsWith('tg-') || userId.startsWith('test-')) {
      console.warn('[Profile] ID de test détecté:', userId);
      return {
        data: {
          id: userId,
          ...
          is_provider: true,
          city: 'Lomé, Togo',
          bio: 'Profil de test — les données réelles seront chargées depuis la base de données.',
          avatar_url: null,
          provider_profiles: [{
            id: `${userId}-provider`,
            specialization: 'Service Test',
            experience_years: 0,
            hourly_rate: 0,
            rating: 0,
            reviews_count: 0,
