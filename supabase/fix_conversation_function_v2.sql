...
BEGIN
  ...
  -- Récupérer l'ID du fournisseur depuis la table profiles
  SELECT id INTO v_provider_id
  FROM public.profiles
  WHERE id::text = p_provider_external_id
     OR provider_external_id = p_provider_external_id
  LIMIT 1;

  IF v_provider_id IS NULL THEN
    RAISE EXCEPTION 'Prestataire non trouvé avec l''ID: ' || p_provider_external_id;
  END IF;

  -- Vérifier si une conversation existe déjà avec ce client et ce fournisseur
  ...
  IF v_conversation_id IS NULL THEN
    -- Créer une nouvelle conversation avec le provider_id
