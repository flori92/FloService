...
BEGIN
  ...
  AND (c.client_id = p_user_id OR c.provider_id = p_user_id);
  
  IF v_conversation_id IS NULL THEN
    RAISE EXCEPTION 'Message non trouvé ou accès non autorisé';
    RETURN FALSE;
  END IF;
  
  -- Ne pas marquer comme lu si c'est l'expéditeur qui fait la demande
...
BEGIN
    IF EXISTS (
        ...
        DROP POLICY "Les utilisateurs peuvent voir leurs conversations" ON conversations;
    END IF;
    ...
    IF EXISTS (
        ...
        WHERE schemaname = 'public' 
        AND tablename = 'conversations' 
        AND policyname = 'Les clients peuvent créer des conversations'
    ) THEN
        DROP POLICY "Les clients peuvent créer des conversations" ON conversations;
    END IF;
    ...
    IF EXISTS (
        ...
        AND policyname = 'Les utilisateurs peuvent voir les messages de leurs conversations'
    ) THEN
        DROP POLICY "Les utilisateurs peuvent voir les messages de leurs conversations" ON messages;
    END IF;
    ...
    IF EXISTS (
        ...
        WHERE schemaname = 'public' 
        AND tablename = 'messages' 
        AND policyname = 'Les utilisateurs peuvent envoyer des messages dans leurs conversations'
    ) THEN
        DROP POLICY "Les utilisateurs peuvent envoyer des messages dans leurs conversations" ON messages;
    END IF;
END
$$;
