-- Migration pour RESTAURER la politique RLS sécurisée sur public.profiles

-- S'assurer que RLS est activé sur la table profiles
ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;

-- Supprimer l'ancienne politique de débogage si elle existe
DROP POLICY IF EXISTS "Allow authenticated read all profiles (debug v4)" ON public.profiles;

-- Supprimer d'autres anciennes politiques au cas où elles traîneraient (pour propreté)
...
-- Recréer la politique RLS standard et sécurisée : les utilisateurs authentifiés peuvent lire leur propre profil.
